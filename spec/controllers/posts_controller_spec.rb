...
#we update how we create my_post so that it belongs to my_topic
  let(:my_post) {my_topic.posts.create!(title: RandomData.random_sentence, body: RandomData.random_paragraph, user: my_user) }


#/////////////BEGIN USER SIGN IN Tests

  #add a context for a guest(un-signed-in) user. Context organizes
  #tests based on the state of an object.
  ...
  #don't allow guests to access create, new, editm update, or destroy
  #methods for any unsigned-in guests
        describe "GET new" do
...
context "Signed-in user" do
  ...
  describe "GET new" do
    it "returns http success" do
  #this first line is set to include the parent topic
      get :new, params: { topic_id: my_topic.id }
      expect(response).to have_http_status(:success)
    end
  ...
  describe "GET show" do
    ...
    describe "DELETE destroy" do
      it "deletes the post" do
        delete :destroy, params: { topic_id: my_topic.id, id: my_post.id }
  #searches the database for post with and an id eqial to my_post.id
  #we assign the size of the array to count and expect it to equal 0
